...
| **An `If` region's border control points come in PAIRS, or not at all.** Inputs cross the border as plain links — no control point. An output needs a Top+Bottom pair joined by `<Link Ids="top,bottom" IsFeedback="true" />` (the splicer), and an unconnected Top must carry a `<p:TypeAnnotation>` (e.g. `SkiaSharp`/`SKImage`) or the type cannot be inferred. An unpaired Top fails with the unhelpful `Unsupported border control point type`; all 35 shipped `If` regions in VL.CoreLib/VL.Skia help are paired. Compiles to `if (cond) { ... } else { x = CreateDefault(); }`. | Prompt Walk across a mountain |
| **`HTTPResult`'s `Split` needs `<CategoryReference Kind="RecordType" Name="HTTPResult" />`** in its NodeReference (category `IO.HTTP.HTTPGet2.HTTPResult`) or the name is ambiguous across 51 records. `Body` is `Spread<Byte>`; `GetInternalArray [Collections.Spread]` turns it into the `Byte[]` that `ImageDecoder [Graphics.Skia.Imaging]` takes. **On the frames before the first response the body is EMPTY and the decoder throws** — gate it with `Count > 0` and an `If`. | Prompt Walk across a mountain |
| **`Pipet [Graphics.Skia.Imaging]` reads one pixel in PIXEL coordinates** (`Position` Vector2, `Enabled`), returning `RGBA` whose channels are 0–1 floats — multiply by 255 and `Round` before treating them as bytes. Out of range reads zeros. `DrawImage [Graphics.Skia.Layers]` (Image, Position, Size, Size Mode, Anchor, Paint, Enabled) shows the same `SKImage`; `Anchor="TopLeft"` as a pin default works. | Prompt Walk across a mountain |
| **Over a Mapsui map, the Skia scene's y axis points DOWN** — a label at y = -0.92 draws at the top of the window. Do not write a y-flip from habit; check where a known pad lands first. The visible width is about 2.47 units at the default window, so x = -1.35 is off-screen. | Prompt Walk across a mountain |
