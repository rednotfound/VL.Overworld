...
| **`FrameDelay` is generic over whatever `Initial Value` is** — `FrameDelay_P<Spread<LineString>>` — and is how a patch holds state that feeds its own next version: Initial Value = seed, Value = this frame's decision, Output = last frame's. With a `Cache` between them the O(n²) step runs once per change, not per frame. | Prompt Grow a town |
