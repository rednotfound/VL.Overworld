...
**Everything listed below is built and committed; all but the newest is rung-4 verified.** Today added
...
own session — **`Prompt Grow a town`: rungs 1–3 passed, rung 4 NOT yet run.** It is the second network
...
"Built 2026-08-28". **Next: run rung 4** (`.\tools\Open-HelpPatch.ps1 "Grow a town"`, press GROW
eleven times, then SAVE; `Normalize-HelpPatches.ps1` after), then mark §4b built in
`vl-nettopologysuite/docs/NETWORK-SCOPE-EVIDENCE.md`, then the Network Package Scope Proposal.
