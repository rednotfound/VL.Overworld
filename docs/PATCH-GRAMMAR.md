...
| **A subpatch is a `Process` definition INSIDE the root `<Canvas>` element** (which then is not self-closed; `Application` follows `</Canvas>`), category `Main`, empty `Create`, pins on the `Update` fragment with `Bounds`, hidden links from pin ids into the canvas; the call site is `LastCategoryFullName="Main"` + `ProcessAppFlag`. A definition beside the canvas instead of inside it is `Not found: <name>`. Full recipe and the survey of shipped use in `docs/SUBPATCHES.md`. | Prompt Grow a town, subpatched |
| **Type every pin of a definition, or the caller's link is dropped in silence.** An untyped pin inferred from a generic inner node becomes `object`, the compiler passes `default(object)`, nothing goes red. `Spread<T>` = `Spread` + `<p:TypeArguments><TypeReference LastCategoryFullName=… ><Choice Kind="TypeFlag" Name="T" /></TypeReference></p:TypeArguments>`; NTS types under `NetTopologySuite.Geometries`. **Rung 3, per definition call site: grep for `default(`.** | Prompt Grow a town, subpatched |
| **Never give a definition an input and an output with the same name** when a script wires it by name — `DeadEnds` `From`/`To` on both sides sent the caller's links to the outputs and the town never grew, with no error anywhere. Found only by drawing stage counts on the picture. | Prompt Grow a town, subpatched |
| **Kill vvvv with `Stop-Process -Force` and confirm the count is 0; `taskkill` from Git Bash failed silently for an hour**, `Open-HelpPatch.ps1`'s "ALREADY RUNNING" refusal went unread, and vvvv hot-reloaded every regenerated file into the stale process. Two rounds of rung 4 were measured against a process started sixty minutes earlier. `Capture-Renderer.ps1` now finds the renderer by enumerating the process's windows, not `MainWindowTitle`; `Open-HelpPatch.ps1 -Log` writes vvvv's log, which is where an exception would be — and in this case there was none, which was itself the finding. | Prompt Grow a town, subpatched |
...
| **A `Path` IOBox resolves against the DOCUMENT; a Path computed at runtime (`ToPath`) resolves against the process working directory** — and silently reads nothing when they differ. A patch that computes file names keeps the folder in a Path *pad* and joins the computed filename with `Combine [IO.Path]` (Path + Path; the string goes through `ToPath` first, which is safe because the pad already anchored the base) | Act IV lab, experiment 3 — the tile reads returned 0 bytes with every status green |
| **`HTTPGet` has a `Headers` pin** — `IEnumerable<string>`, one `"Name: Value"` string per header (a `Cons` of one String feeds it), applied via `Webrequest.AddHeaders` on .NET's `HttpWebRequest`; .NET 8 no longer restricts `Range`, so `"Range: bytes=A-B"` gets a 206 and exactly those bytes. The pin sat unused through five HTTP-fetching units before experiment 4 wired it | Act IV lab experiment 4; first shipped consumer Tutorial 14 |
| **`ToString` on a `String` is ambiguous** — `ToString [Collections.Sequence]` matches a String as a char sequence and vvvvc refuses (`Many compatible implementations`). `Split [HTTPResult]`'s `Status` is already a String: wire it straight to `Text` | experiment 4's prototype, one compile |
| **A hand-placed `Renderer` opens its window in the LIVE session but not on document load** — vvvv serializes its `Bound to Document` pin hidden and VALUELESS (= False), so hot-editing looks fine and every cold open is silently missing a window. A window whose content is a bare map (white) is invisible by absence. Set the pin's `DefaultValue="True"` explicitly — the generator rule from chapter 01's first compile, now confirmed for GUI-placed nodes too | Tutorial 01's second window, 2026-09-23 |
| **A fixed-degree graticule is invisible at the wrong zoom, with every readout healthy.** 10-degree lines are ~1,113 km apart; a zoom-12 window is ~23 km wide, so no line crosses it — nothing renders and no colour change helps. Match `Degrees Spacing` to the view (world view ~10, city view ~0.05), or check the arithmetic before suspecting the node: nearest 10-degree meridian to Tokyo is 27 km away, just outside the window | Tutorial 01, the user's zoom-12 view over a 10-degree grid |
| **A map-side readout sitting at 0,0 is THREE different diseases; `ViewportInfo` tells them apart.** In order: is the window OPEN (`Bound to Document`)? has the map DRAWN once (`ViewportInfo.Width` = 0 means never — the Initial pins apply at first sight of a sized viewport, never again)? is `ScreenToWorld` eating PIXELS (`Console → MouseState`), not the Skia Mouse's canvas units? A fourth, now fixed in the library: `CenterOn` was clamped to the layers' extent on feature-only maps (vl-mapsui NOTES.md 2026-09-23). `ToSkiaLayer`'s `Diagnostics` pin prints viewport size + home-called live | Tutorial 01's map side — all four met in one afternoon |
| **The `Initial` pins are a ONE-SHOT with a deadline, and live hot-editing can miss it.** `Map`'s Initial Center/Zoom apply "once, when the viewport first has a size" (its own docs); build the map chain by hot-editing a RUNNING patch and the viewport got its size before your Home existed — the map sits at Mapsui's factory default, **centre (0,0), 1 m/px**: a ~1 km window of sea off null island, no error anywhere. The numbers give it away (a lon/lat readout of ±0.005 is that 1 km window's edge). Fix: reopen the document cold. Also: `ScreenToWorld` wants WINDOW PIXELS (its docs say "view pixel") — the Skia Mouse's `Position In World` is canvas units, a different child of the same pixel parent; only `Console → MouseState` speaks pixels | Tutorial 01's map side, diagnosed from the readout arithmetic 2026-09-23 |
| **A bool feeds `Switch [Control]`'s `Index` directly** — VL inserts `Convert.ToInt32`, so false picks `Input` and true picks `Input 2`: one node where `Switch (Boolean)` plus its Condition wiring was two. And **one `Spread<Vector2>` IOBox can feed two consumers at once** — a ForEach → `Coordinate` → `LinearRing` for NTS and Skia `Polygon.Points` verbatim — replacing N scalar pads × (Split + Coordinate) + a Cons chain with one pad and one loop. Both from the user's hand-rework of Tutorial 03 (335 lines removed); generators should produce this density, not the expanded form | Tutorial 03, hand-simplified by the user 2026-09-23 |
| **vvvv's save DROPS XML comments** (`<!-- -->` is not part of the document model, so a hand-arranging session silently deletes them). Anything worth saying in a patch goes in a Comment-typed String pad, which vvvv preserves; anything worth keeping for maintainers goes in the repo docs, never in markup comments | Tutorials 01/02, first hand-arranged save, 2026-09-23 |
| **`Console`'s output layer paints an opaque ground — it goes at the BOTTOM of the render `Group`.** At the top it blacks out every layer beneath it, and nothing errors. How high has it at Input 2, above only the map | Tutorial 14 — a fully black window, caught by the camera |
| **A `Keep` loop whose match can be empty must do ALL its reading inside the loop.** `GetSlice` on the empty output hands `default(Feature)` — null — to whatever follows, and one `TryGetValue(null)` per frame kills `Update()` short of the Renderer with every automated rung green. Inside the loop the body simply never runs; results leave via bottom control points (Which door carried six), and the outer `GetSlice`s get `Default Value`s. 'Not found' must stay a COUNT, never become a null | Tutorial 14 — the open state (cursor outside the grid) was the exception path |
| **`FontAndParagraph`'s default text paint is WHITE.** Leave its `Color` unwired and every readout vanishes on a white ground — which is exactly what the consent-off state of a map chapter shows (no basemap = white). Wire an ink pad or a dark `DefaultValue`; How high's `Ink` pad (0.1, 0.1, 0.1, 1) exists for this reason | Explanation Sharper is a different question — a blank white renderer, caught by the camera before any person |
| **A missing file must be an ANSWER, not an exception.** `FileReader` throws per attempt on a nonexistent path — in a patch that computes paths from the cursor this flashes the node purple at every crossing, invisible to rungs 1–3. `Exists [IO.Path]` (node shape like `Combine`: `RecordType Path` + `CategoryReference IO`; pins `Input`/`Exists`) gates the `Read` through `AND`, and a `Switch (Boolean)` hands the parser `''` when there is nothing to read — same zeros on screen, no exception as control flow | Explanation You do not have to read everything — caught by the person at rung 4, after three green rungs |
