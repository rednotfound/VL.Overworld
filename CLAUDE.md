...
`Prompt How high is here` (Terrarium elevation, renamed from `Walk across a mountain`),
`Prompt Which door` (the first of the two consumers `NTS.Experimental.Network` needs before a scope
proposal — it wanted the surface unchanged, and `Nearest Node` was not needed), and — evening, in its
own session — **`Prompt Grow a town`: all four rungs passed.** Rung 4 caught a patch compiled entirely into `__Create__` (the seventh defect the pack has found, the first in operation assignment rather than in a library) — fixed, then FOUND / ROUTE / RESET / SAVE all watched. It is the second network
consumer and `Write GeoJSON`'s first consumer anywhere. Two design corrections were made before it was
opened (a connected seed could never have shown `Found = false`; a grid town cannot shrink below its
Manhattan floor) and are written up with the press-by-press prediction in `docs/ACT-III-DESIGN.md`,
"Built 2026-08-28". §4b is marked built in `vl-nettopologysuite/docs/NETWORK-SCOPE-EVIDENCE.md`. **2026-08-28, night: the Network
...
unchanged from the 08-23 entry below.
...
| ~~review: `NTS.Network`~~ **promoted 2026-08-28** by the Network Package Scope Proposal (`vl-nettopologysuite/docs/NETWORK-SCOPE-PROPOSAL.md`), after both consumers it demanded were rung-4 verified: category `NTS.Network`, node `Network` (was `BuildNetwork`), `ShortestPath` + `Max Snap Distance` (default ∞). The 2026-08-23 decision that parked it under `NTS.Experimental.Network` is recorded there and in that repo's ARCHITECTURE.md | **nothing** |
...
| `D:\2026_Projects\vl-nettopologysuite` | geometry **and the feature model**: create, inspect, operate — and since 2026-08-23 **index** (`NTS.Index`, its first `[ProcessNode]`) plus **`NTS.Network`** (`Network` + `ShortestPath`, an algorithm of ours, promoted from experimental 2026-08-28 by that repo's `docs/NETWORK-SCOPE-PROPOSAL.md`). Category `NTS` | **126** xunit | 13 |
...
3. **Read the generated C#.** This is the rung people skip. A pin fed by nothing compiles as a
   ...
   Check that values arrive from `__pad_…`, that process nodes are constructed in `__Create__`, **that
   no definition call site reads `default(…)` for a pin you wired** (an untyped or misnamed pin drops
   the link in silence), **and that the `.Update(` calls sit inside `Update()`, not inside `__Create__`** (2026-08-28: a computed
